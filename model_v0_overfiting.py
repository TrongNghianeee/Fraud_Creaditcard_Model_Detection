...
# !pip install imbalanced-learn xgboost==2.0.3 pandas numpy==1.26.4 scipy==1.13.1 joblib matplotlib category-encoders cesium --no-deps
...
@dataclass
class FAConfig:
    n_fireflies: int = 30  # Giảm để chạy nhanh hơn
    n_epochs: int = 10     # Giảm để chạy nhanh hơn
    alpha: float = 0.20
    beta0: float = 1.5
    gamma: float = 0.15
    lambda_feat: float = 0.003
    random_state: int = 42
...
def _apply_resampling(X: pd.DataFrame, y: pd.Series, method: int, ratio: float, k_neighbors: int) -> Tuple[pd.DataFrame, pd.Series]:
    return X, y  # Tạm thời tắt resampling để giảm thời gian
    if method == 0 or not HAS_IMBLEARN:
        return X, y
    ratio = float(max(0.1, min(2.0, ratio)))
    k_neighbors = int(max(3, min(20, k_neighbors)))
    try:
...
def evaluate_candidate(
    ...
    X_tr_use, y_tr_use = _apply_resampling(
        ...
        ratio=float(resample_cfg.get("ratio", 1.0)),
    ...
    model = XGBClassifier(
        ...
        eval_metric="aucpr",
        n_estimators=400,
        learning_rate=hyperparams.get("eta", 0.05),
        max_depth=int(hyperparams.get("max_depth", 6)),
        subsample=hyperparams.get("subsample", 0.8),
        colsample_bytree=hyperparams.get("colsample_bytree", 0.8),
        reg_lambda=hyperparams.get("reg_lambda", 2.0),
        min_child_weight=hyperparams.get("min_child_weight", 1.0),
    ...
    try:
        start = time.time()
        model.fit(X_tr_use, y_tr_use, eval_set=[(X_val[cols], y_val)], verbose=False, early_stopping_rounds=30)
        proba = model.predict_proba(X_val[cols])[:, 1]
        ap = float(average_precision_score(y_val, proba))
        print(f"[DEBUG] Candidate eval took {time.time() - start:.2f}s, early stopping at round {model.best_iteration or 400}, AP={ap:.5f}")
        return ap
    except Exception as e:
...
def decode_position(pos: np.ndarray, n_features: int) -> Tuple[np.ndarray, Dict, Dict]:
    ...
    hp = {}
    hp_pad = np.concatenate([hp_vals, np.zeros(max(0, 11 - len(hp_vals)))])
    hp["eta"] = float(scale(hp_pad[0], 0.01, 0.1))
    hp[" max_depth"] = int(round(scale(hp_pad[1], 4, 12)))
    hp["subsample"] = float(scale(hp_pad[2], 0.6, 1.0))
    hp["colsample_bytree"] = float(scale(hp_pad[3], 0.6, 1.0))
    hp["reg_lambda"] = float(scale(hp_pad[4], 1.0, 8.0))
    hp["min_child_weight"] = float(scale(hp_pad[5], 1.0, 10.0))
    hp["max_delta_step"] = int(round(scale(hp_pad[6], 0, 10)))
    raw_method = hp_pad[7]
    method = int(np.clip(np.floor((raw_method + 1) / 2 * 3), 0, 2))
    ratio = float(scale(hp_pad[8], 0.1, 2.0))
    k_neighbors = int(round(scale(hp_pad[9], 3, 20)))
...
def firefly_optimize(
    ...
    for epoch in range(config.n_epochs):
        ...
        if current_best > best_fit_history[-1]:
            best_fit_history.append(current_best)
            best_pop = pop.copy()
        else:
            best_fit_history.append(best_fit_history[-1])
        print(f"[INFO] Epoch {epoch + 1} took {time.time() - start_epoch:.2f}s, best fitness: {current_best:.5f}")
        if len(best_fit_history) > patience and best_fit_history[-1] <= best_fit_history[-patience]:
            print(f"Early stopping FA at epoch {epoch + 1}, best fitness: {best_fit_history[-1]:.5f}")
            break
...
# -----------------------------
# Overfitting prevention helpers
# -----------------------------
def xgb_find_best_nrounds(X: pd.DataFrame, y: pd.Series, params: Dict, num_boost_round: int = 2000, early_stopping_rounds: int = 50, folds: int = 3):
...
def train_fa_xgb(X_tr, y_tr, X_va, y_va, X_te, y_te, seed=42, fa_cfg: Optional[FAConfig] = None, out_dir: str = "outputs"):
    ...
    try:
        best_nrounds, cvres = xgb_find_best_nrounds(X_tr_use, y_tr_use, base_params, num_boost_round=1200, early_stopping_rounds=50, folds=3)
        print(f"[INFO] XGB CV chose {best_nrounds} rounds (approx).")
        try:
            plt.figure(figsize=(8,4))
            if "test-aucpr-mean" in cvres.columns:
                plt.plot(cvres["test-aucpr-mean"], label="test-aucpr-mean")
                plt.plot(cvres["train-aucpr-mean"], label="train-aucpr-mean")
                plt.xlabel("boosting round"); plt.ylabel("aucpr"); plt.legend(); plt.grid(True)
                plt.tight_layout()
                plt.savefig(os.path.join(out_dir, "xgb_cv_aucpr.png"), dpi=150)
                plt.close()
        except Exception as e:
            print(f"[WARN] failed to plot xgb cv: {e}")
    except Exception as e:
        print(f"[WARN] xgb cv failed: {e}; falling back to default n_estimators 800")
        best_nrounds = 800
    ...
    proba_test = clf.predict_proba(X_te[cols])[:, 1]
    ...
    return {"model": clf, "metrics": metrics, "val_pr_auc": float(average_precision_score(y_va, proba_val)), "evals_result": evals_result, "checkpoint": model_checkpoint_path}
